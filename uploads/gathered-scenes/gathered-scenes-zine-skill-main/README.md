...
- **真景为锚**：真实场景不是装饰素材，而是作品唯一的事实基础。
- **同景转译**：摄影与插画共享同一视点和空间坐标，只改变媒介与细节，不另造场景。
...
| **转化方式** | 在同一视点与空间坐标中分配摄影和插画媒介，保持对象、透视、尺度与光线连续 | 从事实中提取命题、张力与视觉隐喻，重新创作 |
| **结果** | 同一真实场景跨越纸张边界连续转译的克制拼贴海报 | 可脱离原场景、表达优先的极简编辑插画 |
| **调用名称** | `$scenes-gathered-zine-v1-3` | `$scene-distillation-zine-v1-3` |

### 01 · 实景拼贴

`scenes-gathered-zine-v1-3` 把照片作为唯一事实来源，让摄影与抽象插画在同一视点、同一透视和同一空间坐标中连续衔接。对象身份、位置、尺度、遮挡与光线保持不变，只让视觉媒介、细节密度、单一高纯度色彩和手撕纸张质感发生转译。
...
| **03 · 转译** | 在原场景坐标中简化轮廓、路径、光影与细节，只改变纸上媒介 |
| **04 · 编排** | 配准摄影与插画边界，让对象、透视、遮挡和光线连续，再组织文字与留白 |
...
保留桥上人群、水面倒影与雪林的原始位置和空间层次，让它们在同一坐标中从摄影连续转为安静的蓝灰纸上媒介，使横向行进成为整张作品的节奏。[查看观察记录](examples/real-scene-collage/02-winter-crossing/)
